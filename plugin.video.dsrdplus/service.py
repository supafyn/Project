...
from dsrd_lib import image_proxy
from dsrd_lib import utils2
from dsrd_lib.utils2 import i18n
from dsrd_lib.constants import MODES
from dsrd_lib.db_utils import DB_Connection
from dsrd_lib.trakt_api import Trakt_API
...
class Service(xbmc.Player):
    ...
    def reset(self):
        ...
        self.win.clearProperty('dsrdplus.playing')
        self.win.clearProperty('dsrdplus.playing.trakt_id')
        self.win.clearProperty('dsrdplus.playing.season')
        self.win.clearProperty('dsrdplus.playing.episode')
        self.win.clearProperty('dsrdplus.playing.srt')
        self.win.clearProperty('dsrdplus.playing.trakt_resume')
        self.win.clearProperty('dsrdplus.playing.salts_resume')
        self.win.clearProperty('dsrdplus.playing.library')
    ...
    def onPlayBackStarted(self):
        ...
        playing = self.win.getProperty('dsrdplus.playing') == 'True'
        self.trakt_id = self.win.getProperty('dsrdplus.playing.trakt_id')
        self.season = self.win.getProperty('dsrdplus.playing.season')
        self.episode = self.win.getProperty('dsrdplus.playing.episode')
        srt_path = self.win.getProperty('dsrdplus.playing.srt')
        trakt_resume = self.win.getProperty('dsrdplus.playing.trakt_resume')
        salts_resume = self.win.getProperty('dsrdplus.playing.salts_resume')
        self._from_library = self.win.getProperty('dsrdplus.playing.library') == 'True'
    ...
    if kodi.has_addon('plugin.program.super.favourites'):
        ...
        if active_plugin == kodi.get_id():
            if sf.getSetting('CONTEXT') == 'true':
                logger.log('Disabling Global CX while Death Streams RD is active', log_utils.LOGDEBUG)
                was_on = True
                sf.setSetting('CONTEXT', 'false')
        elif was_on:
            logger.log('Re-enabling Global CX while Death Streams RD is not active', log_utils.LOGDEBUG)
