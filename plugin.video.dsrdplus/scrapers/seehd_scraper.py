...
from dsrd_lib.constants import QUALITIES
...
class Scraper(scraper.Scraper):
    ...
    def search(self, video_type, title, season=''):  # @UnusedVariable
        search_url = scraper_utils.urljoin(self.base_url, '/search/')
        search_url = search_url % (urllib.quote_plus(title))
        html = self._http_get(search_url, self.base_url, params={'s': title}, cache_limit=2)
        pattern = 'class="entry-title">\s*<a[^>]+href="(?P<url>[^"]+)[^>]*>\s*(?P<post_title>[^<]+)'
        return self._blog_proc_results(html, pattern, '', video_type, title)
