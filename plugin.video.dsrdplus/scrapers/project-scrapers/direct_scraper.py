...
class Scraper(scraper.Scraper):
    ...
    def get_sources(self, video):
        ...
        if 'error' in js_result:
            logger.log('Direct API error: "%s" @ %s' % (js_result['error'], url), log_utils.LOGWARNING)
            return hosters
    ...
    def search(self, video_type, title, year, season=''):  # @UnusedVariable
        ...
        if 'error' in js_result:
            logger.log('Direct API error: "%s" @ %s' % (js_result['error'], search_url), log_utils.LOGWARNING)
            return results
