"""
    ...
    Copyright (C) 2014 tknorris / 2017 Mr Blamo.Blamo additional credits.
...
from dsrd_lib.db_utils import DB_Connection, DatabaseRecoveryError
from dsrd_lib.srt_scraper import SRT_Scraper
from dsrd_lib.trakt_api import Trakt_API, TransientTraktError, TraktNotFoundError, TraktError, TraktAuthError
from dsrd_lib import salts_utils
from dsrd_lib import utils2
from dsrd_lib import gui_utils
from dsrd_lib import strings
from dsrd_lib import worker_pool
from dsrd_lib import image_scraper
from dsrd_lib.constants import *  # @UnusedWildImport
from dsrd_lib.utils2 import i18n
...
#   kodi.create_item({'mode': MODES.AUTO_CONF}, i18n('auto_config'), thumb=utils2.art('settings.png'), fanart=utils2.art('fanart.jpg'), is_folder=False, is_playable=False)
    ...
    kodi.create_item({'mode': MODES.AUTH_TRAKT}, i18n('auth_dsrealdebrid'), thumb=utils2.art('settings.png'), fanart=utils2.art('fanart.jpg'), is_folder=False, is_playable=False)
    ...
    kodi.create_item({'mode': MODES.SETTINGS}, 'Death Streams Real Debrid By Mr Blamo.Blamo', thumb=utils2.art('settings.png'), fanart=utils2.art('fanart.jpg'))
    kodi.set_content(CONTENT_TYPES.ADDONS)
    kodi.end_of_directory()
...
def get_sources(mode, video_type, title, year, trakt_id, season='', episode='', ep_title='', ep_airdate=''):
    ...
    with kodi.ProgressDialog(i18n('getting_sources'), utils2.make_progress_msg(video), active=active) as pd:
        ...
        try:
            ...
            if not hosters:
                logger.log('Death Fucked Your Sources for: |%s|' % (video), log_utils.LOGWARNING)
                msg = i18n('Death_fucked_ur_sources')
...
def play_source(mode, hoster_url, direct, video_type, trakt_id, season='', episode=''):
    ...
    with kodi.WorkingDialog() as wd:
        ...
        win.setProperty('dsrdplus.playing', 'True')
        win.setProperty('dsrdplus.playing.trakt_id', str(trakt_id))
        win.setProperty('dsrdplus.playing.season', str(season))
        win.setProperty('dsrdplus.playing.episode', str(episode))
        win.setProperty('dsrdplus.playing.library', str(from_library))
        if resume_point > 0:
            if kodi.get_setting('trakt_bookmark') == 'true':
                win.setProperty('dsrdplus.playing.trakt_resume', str(resume_point))
            else:
                win.setProperty('dsrdplus.playing.salts_resume', str(resume_point))
    ...
    with kodi.WorkingDialog() as wd:
        ...
        if video_type == VIDEO_TYPES.EPISODE and utils2.srt_download_enabled() and show_meta:
            ...
            if utils2.srt_show_enabled() and srt_path:
                ...
                win.setProperty('dsrdplus.playing.srt', srt_path)
